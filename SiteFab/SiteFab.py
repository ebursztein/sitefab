...
class SiteFab(object):
    ...
    def __init__(self, config_filename, version='1.0'):
        ...
        # [parser] #
        self.config.parser = Parser.make_config(
            self.config.parser)  # initialize the parser config

        # [plugins] #
        plugins_config_filename = os.path.join(
            files.get_site_path(), self.config.plugins_configuration_file)
        plugins_config = files.load_config(plugins_config_filename)

        # where to redirect the standard python log
        debug_log_fname = os.path.join(self.get_logs_dir(), "debug.log")
        ...
        cfg.template_dir = os.path.join(
            files.get_site_path(),  self.config.logger.template_dir)
        ...
        # [linter] #
        linter_config_filename = os.path.join(
            files.get_site_path(), self.config.linter.configuration_file)
        linter_config = files.load_config(linter_config_filename)
        linter_config.report_template_file = os.path.join(
            files.get_site_path(), self.config.linter.report_template_file)
        ...
        self.filenames.posts = files.get_files_list(
            self.get_content_dir(), "*.md")
    ...
    def parse(self):
        ...
        path = os.path.join(self.get_output_dir(),
                            self.config.collections.output_dir)
        self.posts_by_tag = PostCollections(
            site=self, template=tlp, output_path=path, web_path=self.config.collections.output_dir, min_posts=min_posts)
    ...
    def finale(self):
        ...
        preprocessing_ts = round(
            self.timings.preprocessing_stop - self.timings.preprocessing_start, 2)

...
def get_normalized_list_of_words(txt, stop_words, word_min_letters=2,
                                 word_max_letters=16):
    """ Perform all the normalization steps in one pass
    Args:
        txt (str): text to normalize
        stop_words (set): set of stop words to remove
        word_min_letters (int): How many letter a word should be have at least.
        word_max_letters (int): How many letter a word should have at most.
...
def build_stop_words(lang='en'):
    ...
    stop_words += ['can', 'will', 'use', 'one',
                   'using', 'used', 'also', 'see', 'first', 'like']
    stop_words += ['page', 'get', 'new', 'two',
                   'site', 'blog', 'many', "don't", 'dont', 'way']
    stop_words += ['last', 'best', 'able', 'even',
                   'next', 'last', 'let', "none", 'every', 'three']
    stop_words += ['lot', 'well', 'chart', 'much',
                   'based', 'important', 'posts', 'reads', 'least']

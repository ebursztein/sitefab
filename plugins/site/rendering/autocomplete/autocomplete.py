...
class Autocomplete(SiteRendering):
    ...
    def process(self, unused, site, config):
        ...
        for post in site.posts:
            ...
            for gram_len, grams in nlp.grams.items():
        ...
        if not total_doc:
            return (SiteFab.ERROR, plugin_name, 'no documents')
        ...
        for val, score in other_fields.items():
            suggestions[val] = score

        output = []
        log_info += "num of ngram considered: %s<br>" % len(ngrams_frequencies)
        log_info += """<table><tr><th>ngram</th><th>score</th>
        <th>post frequency</th><th>tf-idf score</th></tr>"""
        for info in suggestions.most_common(num_suggestions):
            ...
            log_info += """<tr><td>%s</td><td>%s</td><td>%s</td>
                    <td>%s</td></tr>""" % (word, score, post_freq, ngram_freq)
